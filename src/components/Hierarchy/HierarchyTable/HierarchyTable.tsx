...
const HierarchyTable: FC<HierarchyTableProps> = props => {
	...
	const stickyHeaderStyle = stickyHeader ? { position: 'sticky', top: 0, zIndex: 1 } : {};

	const getRowBgColor = (index: number): string => (index % 2 === 0 ? evenRowBg : oddRowBg);
		...
		<>
			<Table size='small'>
				<TableHead sx={{ backgroundColor: headerBg, ...stickyHeaderStyle }}>
					<TableRow>
						<TableCell>{headerFirstCell}</TableCell>
